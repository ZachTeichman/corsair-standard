...
def parse_docx(path: str | Path) -> DocumentModel:
    ...
    for index, para in enumerate(body_paragraphs):
        ...
        layout_text = text.rstrip(" \t")
        normalized_for_spacing = re.sub(r" *\t *", "\t", layout_text)
        stripped = text.lstrip(" ")
        leading_spaces = len(text) - len(stripped)
        raw_repeated_spaces = bool(re.search(r" {2,}", layout_text))
        tab_space_padding = bool(re.search(r"(?: {2,}\t|\t {2,})", layout_text))
